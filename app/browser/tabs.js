...
const api = {
  init: (state, action) => {
    ...
    app.on('web-contents-created', function (event, tab) {
      ...
      tab.on('did-attach', (e, tabId) => {
        console.log('browser/tabs.js::: did-attach:', tabId)
      })

      tab.on('did-detach', (e, tabId) => {
        console.log('browser/tabs.js::: did-detach', tabId)
      })

      tab.on('guest-ready', (e, tabId, guestInstanceId) => {
        console.log('browser/tabs.js::: guest-ready', tabId)
  ...
  detachGuest: (state, action) => {
    ...
    if (tabValue) {
      ...
      console.log('tabs.js:::: detachGuest found win id:', win.id)
      win.setTabIdPendingTransfer(tabId)
    }
    const tab = api.getWebContents(tabId)
    if (tab && !tab.isDestroyed()) {
      tab.detachGuest()
